...
int main()
{
	...
	while(1)
	{
		...
		if(flag==1)
		{
			...
			for(i=0;i<reuse;i++)
            {
                canuse=canuse+use[i][1];
            }
			if(size<=canuse)
            {
                cout<<"内存中所有空闲区总和大于等于新进程大小"<<endl;
            s:  cout<<"请选择寻找空闲区的算法，1.最先适应算法、2.最佳适应算法、3.最坏适应算法"<<"  ";
			    cin>>choose;
			switch(choose)
			{
			    case 1:
					{
						...
						int z=0;
                            for(i=0;i<reuse;i++)
                            {
								for(int t=use[i][0];t<use[i][0]+use[i][1];t++)
								{
								    if(z<size){memory[t]=n;z++;}
								    else{break;}
								}
						    }
						usedprint(memory);
            ...
            {
                cout<<"内存中所有空闲区总和小于新进程大小，将淘汰老段"<<endl;
                cout<<"淘汰算法：1、先进先出；2、LRU算法。"<<"  ";
                int none;
                cin>>none;
            st: int minp=n;
                for(i=0;i<m;i++)
                {
                    if(memory[i]!=0)
                    {
                        if(minp>memory[i]){minp=memory[i];}
                    }
                }
                cout<<"淘汰第"<<minp<<"号段"<<endl;
                for(i=0;i<m;i++)
                {
                    if(memory[i]==minp){memory[i]=0;}
                }
                usedprint(memory);
                reuse=unusedprint(memory,use);
                int canuse=0;
